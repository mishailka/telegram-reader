...
Folder tools are available in v0.3: list_folders, inspect_folder, analyze_chats_for_folders, suggest_folder_plan, preview_folder_changes, create_folder, update_folder, set_folder_chats, reorder_folders, delete_folder, list_folder_backups, restore_folder_backup and apply_folder_plan. Analyze, preview and backup listing are read-only. Folder mutation changes Telegram on all clients, so call it only after the user explicitly asks to apply the reviewed plan. Each mutation creates a DPAPI-protected local snapshot; use restore_folder_backup when the user asks to undo a folder change. Never change folder IDs 0 (main) or 1 (Archive), and never reveal folder peers outside the selected chat scope. Use recent-message sampling only when it is relevant and requested. A new Codex task may be needed after installation for tools to appear.
